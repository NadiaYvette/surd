...
pslq xs maxIter
  ...
  | normX < 1e-20 = Nothing
  | otherwise =
      -- Reify dimensions: n rows, (n-1) columns for H; n×n for A, B.
  ...
  where
    ...
    go y h a b iter
      ...
      | otherwise =
          ...
          -- After the row swap, H[m][m+1] may be non-zero. Apply a Givens
          -- rotation to columns m and m+1 to zero it out.
              h2 = if pivotM < n - 2
                   then let hA = h1 ! (pivotM, pivotM)
                            hB = h1 ! (pivotM, pivotM + 1)
                            (co, si) = givensRotation hA hB
                        -- Apply rotation to columns pivotM and pivotM+1.
                        -- G^T acts on columns: [col_m, col_{m+1}] *= G
...
-- | Safe column index for H (n × (n-1)): returns 0 for out-of-bounds.
