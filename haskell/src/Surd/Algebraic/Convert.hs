   ...
   in if q == 0
        then -- Biquadratic: t⁴ + pt² + r = 0, solve as quadratic in t²
          ...
          let rcCs = [4 * p * r - q * q, -4 * r, -p, 1]
           in -- Use solveCubic to get a resolvent root
              -- We need any root, so use a dummy approx (we'll try all later)
              case solveCubic rcCs 0 of
                Nothing -> Nothing
                Just yExpr ->
  ...
  where
    ...
    showTerm termIdx power coeff =
      let sign
          ...
          varStr
            | power == 0 = if coeffStr == "" then "1" else coeffStr
            | power == 1 = coeffStr ++ "x"
