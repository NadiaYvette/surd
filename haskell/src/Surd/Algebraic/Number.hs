...
data AlgNum = AlgNum
  { anMinPoly  :: !(Poly Rational)   -- ^ Monic irreducible minimal polynomial
  , anInterval :: !Interval          -- ^ Isolating interval (rational endpoints)
  } deriving (Show)

-- | Embed a rational number as an algebraic number.
algFromRational :: Rational -> AlgNum
algFromRational r = AlgNum
...
pickBestRoot factors approx =
  let candidates = concatMap (\f -> map (\ii -> (f, ii)) (isolateRealRoots f)) factors
      scored = [(f, ii, abs (fromRational mid - approx))
  ...
  in case scored of
       [] -> Nothing
       _  -> let (f, iv, _) = minimumBy (\(_, _, d1) (_, _, d2) -> compare d1 d2) scored
             in Just (AlgNum f iv)
  where
    minimumBy _ [x] = x
