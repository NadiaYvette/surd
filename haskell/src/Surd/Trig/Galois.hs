...
data PeriodState = PeriodState
  { -- | Radical expression for this period
    periodExpr :: !(RadExpr Rational),
    -- | Which exponents k (in ζ^k) this period sums
    periodElems :: ![Integer],
    -- | The prime p
    periodP :: !Integer
  }
...
splitPeriod ::
  -- | All periods at the current level
  [PeriodState] ->
  -- | Prime index q
  Int ->
  -- | The period to split
   ...
   in if magnitude (target - (fromIntegral nearestInt :+ 0)) < 1e-8
        then Just (Lit (fromIntegral nearestInt))
          ...
          let periodVals = map (sumRootsOfUnity p . periodElems) periods
           ...
           in -- Short-circuit: if single-period matching found something, use it.
              -- This avoids the expensive multi-period backtracking search.
              case singleMatches of
                (m : ms) ->
...
matchSinglePeriod target v =
  ...
  where
    pickCoeff w
      | abs (imagPart w) > 1e-10 = round (imagPart target / imagPart w) :: Int
      | abs (realPart w) > 1e-10 = round (realPart target / realPart w)
      | otherwise = 0
...
findIntegerComboC target vals =
  solveLinearIntegerC
    (toRational (realPart target), toRational (imagPart target))
...
solveLinearIntegerC (tRe, tIm) vals =
  let n = length vals
      ...
      (imHeavy, realOnly) =
        foldr
          ( \iv@(_, (_, im)) (ih, ro) ->
              if abs im > 1e-30 then (iv : ih, ro) else (ih, iv : ro)
          )
          ([], [])
          indexed
...
solvePeriodEquation ::
  ...
  RadExpr Rational ->
  -- | [e₁, e₂, ..., eₙ] as radical exprs
  [RadExpr Rational] ->
  -- | Numerical values of sub-periods (complex)
  [Complex Double] ->
  [RadExpr Rational]
...
solvePeriodEquation _q _e1 _coeffExprs numVals =
  ...
  map (Lit . toRational . realPart) numVals
...
solvePeriodViaResolvent ::
  -- | Degree q (prime, ≥ 5)
  Int ->
  -- | All current-level periods
  [PeriodState] ->
  -- | p (the prime)
  Integer ->
  -- | Parent period expression (= R_0)
  RadExpr Rational ->
  -- | Sub-period element lists
  [[Integer]] ->
  -- | Numerical sub-period values
  [Complex Double] ->
  [RadExpr Rational]
solvePeriodViaResolvent q allPeriods p parentExpr subPeriodElems _numVals =
  let -- Compute high-precision DFT via MPBall (1000-bit precision).
      ...
      dExprs =
        [ case solveLinearIntegerC (dCoeffsHP !! s) periodValsD of
            Just (c, coeffs) ->
              foldl
                Add
                (Lit (fromIntegral c))
      ...
      resolventPowerExprs =
        [ foldl1
            Add
            [ Mul (dExprs !! s) (omegaPowers !! ((j * s) `mod` q))
              | s <- [0 .. q - 1]
            ]
          | j <- [1 .. q - 1]
        ]
      ...
      subPeriodExprs =
        [ Mul
            (Inv (Lit (fromIntegral q)))
            ( foldl1
                Add
                [ Mul
                    (omegaPowers !! ((q - ((j * k) `mod` q)) `mod` q))
                    (allResolvents !! j)
                  | j <- [0 .. q - 1]
                ]
            )
   ...
   in -- Sub-period expressions are already in correct order by DFT construction:
  ...
  where
    selectBranch rjqVal =
      let omegaC = exp (0 :+ (2 * pi / fromIntegral q))
          principalVal =
            mkPolar
              (magnitude rjqVal ** (1 / fromIntegral q))
              (phase rjqVal / fromIntegral q)
          scored =
            [ (k, magnitude (omegaC ^ k * principalVal - targetVal))
              | k <- [0 .. q - 1]
            ]
  ...
  where
    pickClosest evs target =
      case evs of
        ...
        _ -> fst $ minimumBy (comparing (\(_, v) -> magnitude (v - target))) evs
