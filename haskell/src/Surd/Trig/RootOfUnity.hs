...
cosOfUnityPrimePower :: Int -> Int -> Maybe (RadExpr Rational)
cosOfUnityPrimePower p k
  | p == 2    = Just (cosOfPow2 (2 ^ k))
  | otherwise =
      -- cos(2π/p^k): relates to cos(2π/p^{k-1}) via:
      -- Φ_{p^k}(x) = Φ_p(x^{p^{k-1}}) / Φ_p(x^{p^{k-2}})  ... not quite.
      -- Actually, cos(2π/p²) satisfies a degree-p polynomial over Q(cos(2π/p)).
      -- For now, use the Gauss period descent on p^k directly.
      -- The group (Z/p^k Z)* is cyclic of order p^{k-1}(p-1).
      --
      -- TODO: implement this via the prime-power Gauss period descent.
      -- For now, handle via the general composite path if possible.
      cosOfUnityComposite (p ^ k)
