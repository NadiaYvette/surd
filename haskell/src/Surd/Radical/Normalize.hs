...
collectCoefficients = go
  where
    ...
    partitionLits (Inv (Lit r) : xs) | r /= 0 =
...
collectTerms = go
  where
    ...
    groupTerms = foldl (\m term ->
      let (c, base) = splitCoeff term
      in Map.insertWith (+) base c m) Map.empty
...
sortCommutative = go
  where
    ...
    buildAdd' []     = Lit (fromInteger 0)  -- shouldn't happen
    buildAdd' [x]    = x
    buildAdd' (x:xs) = foldl Add x xs

    buildMul' []     = Lit (fromInteger 1)  -- shouldn't happen
    buildMul' [x]    = x
