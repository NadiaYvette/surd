...
import Math.Field.Extension (ExtElem (..), ExtField (..), embed, generator, mkExtField)
import Math.Field.Transcendental
import Math.Polynomial.Multivariate
import Math.Polynomial.Univariate (Poly (..), divModPoly, mkPoly)
import Math.Polynomial.Univariate qualified as U
