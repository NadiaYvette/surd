...
tests =
  testGroup
    "Trig"
    [ testGroup
        "Constructible (square roots only)"
        [ testCase "cos(0) = 1" $ do
      ...
      testGroup
        "Non-constructible (via Gauss period descent)"
      ...
      testGroup
        "Rigorous interval verification (rational)"
      ...
      testGroup
        "MPBall arbitrary-precision eval"
