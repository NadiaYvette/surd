...
basicArithTests = testGroup "basic rational arithmetic"
...
extensionTests = testGroup "simple extension Q(√2)"
  [ testCase "√2 · √2 = 2" $ do
      -- Adjoin √2: α² = 2
...
inversionTests = testGroup "field inversion"
  [ testCase "1/√2 · √2 = 1" $ do
      let (_, alpha) = adjoinTowerRoot 1 2 (TRat 2)
...
nestedTests = testGroup "nested tower Q(√2)(√3)"
...
conversionTests = testGroup "tower to RadExpr conversion"
...
towerDescentTests = testGroup "tower descent"
      ...
      case cosViaTower 5 of
        ...
        Just expr -> do
          let val = dagEvalComplex (toDAG expr)
              expected = cos (2 * pi / 5)
          assertBool ("cos(2π/5): " ++ show val ++ " vs " ++ show expected)
  ...
  , testCase "cosViaTower 7 RadExpr is numerically correct" $ do
      case cosViaTower 7 of
        ...
        Just expr -> do
          let val = dagEvalComplex (toDAG expr)
              expected = cos (2 * pi / 7)
          assertBool ("cos(2π/7): " ++ show val ++ " vs " ++ show expected)
                     (abs (realPart val - expected) < 1e-10)
  ...
  , testCase "cosViaTower 11 RadExpr is numerically correct" $ do
      case cosViaTower 11 of
        ...
        Just expr -> do
          ...
          assertBool ("cos(2π/11): " ++ show val ++ " vs " ++ show expected)
  ...
  , testCase "cosViaTower 13 RadExpr is numerically correct" $ do
      case cosViaTower 13 of
        ...
        Just expr -> do
          ...
          assertBool ("cos(2π/13): " ++ show val ++ " vs " ++ show expected)
                     (abs (realPart val - expected) < 1e-8)
  ]
